...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,600;0,700;1,400&family=Plus+Jakarta+Sans:ital,wght@0,300;0,400;0,500;0,600;0,700;1,400&display=swap"
        rel="stylesheet">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-20">
                ...
                <a href="{{ route('home') }}" class="flex items-center space-x-3 group">
                    ...
                    <div
                        class="w-10 h-10 rounded-full overflow-hidden shadow-md shadow-[#0D5C75]/20 group-hover:scale-105 transition-transform duration-300">
                        <img src="{{ asset('storage/image/logo.jpeg') }}" alt="Santorini Logo"
                            class="w-full h-full object-cover">
                    </div>
                    <div>
                        <span class="font-serif text-2xl font-bold tracking-wide text-[#0D5C75]">Santorini</span>
                        <span class="text-xs uppercase tracking-widest block font-semibold text-[#C5A880]">Spa &
                            Wellness</span>
                    </div>
                </a>
                    ...
                    @auth
                        @if (auth()->user()->role === 'admin')
                            <a href="{{ route('admin.dashboard') }}"
                                class="inline-flex items-center px-4 py-2 text-sm font-semibold rounded-full border border-[#0D5C75]/20 text-[#0D5C75] bg-white hover:bg-sky-50 transition-all duration-300 shadow-sm shadow-[#0D5C75]/5">
                        ...
                        @else
                            <a href="{{ route('customer.dashboard') }}"
                                class="inline-flex items-center px-4 py-2 text-sm font-semibold rounded-full border border-[#0D5C75]/20 text-[#0D5C75] bg-white hover:bg-sky-50 transition-all duration-300 shadow-sm shadow-[#0D5C75]/5">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 text-[#C5A880]" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="inline">
                            @csrf
                            <button type="submit"
                                class="inline-flex items-center px-4 py-2 text-sm font-semibold rounded-full text-white bg-rose-500 hover:bg-rose-600 transition-all duration-300 shadow-sm">
                                Keluar
                            </button>
                        </form>
                    @else
                        <a href="{{ route('login') }}"
                            class="text-sm font-semibold text-slate-600 hover:text-[#0D5C75] transition-colors">
                            Masuk
                        </a>
                        <a href="{{ route('register') }}"
                            class="inline-flex items-center px-4 py-2 text-sm font-semibold rounded-full bg-[#0D5C75] text-white hover:bg-[#0A475B] transition-all duration-300 shadow-sm shadow-[#0D5C75]/10">
                            Daftar
                        </a>
                    @endauth
                </div>
    ...
    <main class="flex-grow">
        @if (session('success'))
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
                <div
                    class="p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-2xl flex items-center shadow-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3 text-emerald-600 flex-shrink-0"
    ...
    <footer class="bg-white border-t border-sky-100 text-slate-600 mt-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <!-- Address -->
                <div class="space-y-4">
                    <h3 class="text-sm uppercase tracking-widest font-bold text-[#C5A880]">Alamat Toko Kami</h3>
                    <p class="text-sm text-slate-500 leading-relaxed">
            ...
            <div
                class="border-t border-sky-50 mt-12 pt-8 flex flex-col md:flex-row items-center justify-between text-xs text-slate-400">
