<x-guest-layout>
    ...
    <div x-data="{ role: 'customer' }" class="space-y-6">
        <!-- Dynamic Header -->
        <div>
            <h3 class="font-serif text-2xl font-bold text-slate-800" x-text="role === 'admin' ? 'Masuk Portal Admin' : 'Masuk Akun Pelanggan'">Masuk Akun Pelanggan</h3>
            <p class="text-xs text-slate-400 mt-1" x-text="role === 'admin' ? 'Silakan masukkan email dan password admin Anda' : 'Silakan masuk untuk melakukan booking dan melihat riwayat'">Silakan masuk untuk melakukan booking dan melihat riwayat</p>
        </div>
        ...
        <div class="flex bg-slate-100 p-1 rounded-xl select-none">
            <button type="button" 
                @click="role = 'customer'" 
                :class="role === 'customer' ? 'bg-[#0D5C75] text-white shadow-sm' : 'text-slate-500 hover:text-slate-800 bg-transparent'" 
                class="flex-1 py-2 text-xs font-bold rounded-lg transition-all duration-300">
                Pelanggan
            </button>
            <button type="button" 
                @click="role = 'admin'" 
                :class="role === 'admin' ? 'bg-[#0D5C75] text-white shadow-sm' : 'text-slate-500 hover:text-slate-800 bg-transparent'" 
                class="flex-1 py-2 text-xs font-bold rounded-lg transition-all duration-300">
                Admin
            </button>
        </div>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            <div class="space-y-1.5">
                ...
                <div class="relative flex items-center">
                    ...
                    <input 
                        ...
                        :placeholder="role === 'admin' ? 'admin@santorinispa.com' : 'email@pelanggan.com'"
            ...
            <div class="pt-2">
                <button type="submit" class="w-full py-4 bg-[#0D5C75] hover:bg-[#0A475B] text-white font-bold text-xs uppercase tracking-widest rounded-xl transition-all duration-300 shadow-md shadow-[#0D5C75]/15 flex items-center justify-center">
                    <span x-text="role === 'admin' ? 'Masuk Portal Admin' : 'Masuk Santorini Spa'">Masuk Santorini Spa</span>
                </button>
            </div>

            <div class="text-center text-xs text-slate-500 pt-2" x-show="role === 'customer'">
                Belum punya akun? <a href="{{ route('register') }}" class="font-bold text-[#0D5C75] hover:underline">Daftar sekarang</a>
            </div>
        </form>
    </div>
</x-guest-layout>
